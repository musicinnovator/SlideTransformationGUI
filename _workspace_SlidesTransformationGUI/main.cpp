// Slides Transformation Tool - Main Entry Point
// This file provides the main function and platform-specific entry points
// for the Slides Transformation tool.
...
// Forward declarations of functions from SlidesTransformation.cpp
struct AppState {
...
const char* WINDOW_TITLE = "Slides Transformation Tool";

// Define colors for the UI
#ifdef PLATFORM_WINDOWS
// Medium blue background and light blue button colors for Windows
#define BACKGROUND_COLOR RGB(65, 105, 225) // Medium blue
#define BUTTON_COLOR RGB(135, 206, 250)    // Light blue
#elif defined(PLATFORM_LINUX)
// X11 color values for medium blue and light blue
#define BACKGROUND_COLOR 0x4169E1 // Medium blue
#define BUTTON_COLOR 0x87CEFA    // Light blue
#endif
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Create a medium blue background brush
    wc.hbrBackground = CreateSolidBrush(BACKGROUND_COLOR);
    wc.lpszClassName = "SlidesTransformationClass";
    RegisterClassEx(&wc);
    ...
    HWND hwnd = CreateWindowEx(
        0, "SlidesTransformationClass", WINDOW_TITLE,
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            HBRUSH buttonBrush = CreateSolidBrush(BUTTON_COLOR);
            SetClassLongPtr(hButton1, GCLP_HBRBACKGROUND, (LONG_PTR)buttonBrush);
            ...
            // Set button color to light blue
            SetClassLongPtr(hButton2, GCLP_HBRBACKGROUND, (LONG_PTR)buttonBrush);
            ...
            // Set button color to light blue
            SetClassLongPtr(hButton3, GCLP_HBRBACKGROUND, (LONG_PTR)buttonBrush);
            ...
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"STTM2m - Slide m3 below, M2 up");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"STTm2M - Slide m3 below, m2 up");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"ISTTM2m - Slide M3 above, M2 down");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"DSTTM2m - Dotted slide m3 below, M2 up");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"TTSM2m2M - Three-tone slide P4 below");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"TTSd1M2m2M - Dotted three-tone slide");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"TTITM2M2M - Inverted slide M2 below");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"ITTITM2M2M - Inverted slide M2 above");
            SendMessage(hComboBox, CB_ADDSTRING, 0, (LPARAM)"ITTSM2M2m - Inverted three-tone slide");
            ...
            // Set button color to light blue
            SetClassLongPtr(hButton6, GCLP_HBRBACKGROUND, (LONG_PTR)buttonBrush);
            ...
            // Set button color to light blue
            SetClassLongPtr(hButton7, GCLP_HBRBACKGROUND, (LONG_PTR)buttonBrush);
            ...
            // Title label
            CreateWindow(
                "STATIC", "Slides Transformation Tool", WS_VISIBLE | WS_CHILD | SS_CENTER,
                200, 20, 400, 30, hwnd, NULL, NULL, NULL
            );

            // Description label
            CreateWindow(
                "STATIC", "Transforms eligible notes (SAN, RLN, SMP, I8, U2R, RDN, RN, LAD, LNSN, LNSAS, LNSAL, DI, SPCM, and SPDM with slide patterns",
            ...
            switch (wmId) {
                ...
                case 6: { // Process File
                    ...
                    if (selectedIndex == 0) {
                        ...
                        state->selectedVariants.push_back("STTM2m");
                    } else if (selectedIndex == 2) {
                        state->selectedVariants.push_back("STTm2M");
                    } else if (selectedIndex == 3) {
                        state->selectedVariants.push_back("ISTTM2m");
                    } else if (selectedIndex == 4) {
                        state->selectedVariants.push_back("DSTTM2m");
                    } else if (selectedIndex == 5) {
                        state->selectedVariants.push_back("TTSM2m2M");
                    } else if (selectedIndex == 6) {
                        state->selectedVariants.push_back("TTSd1M2m2M");
                    } else if (selectedIndex == 7) {
                        state->selectedVariants.push_back("TTITM2M2M");
                    } else if (selectedIndex == 8) {
                        state->selectedVariants.push_back("ITTITM2M2M");
                    } else if (selectedIndex == 9) {
                        state->selectedVariants.push_back("ITTSM2M2m");
                    }
        ...
        case WM_CTLCOLORSTATIC: {
            // Set text color for static controls to white for better contrast on blue background
...
int main(int argc, char* argv[]) {
    ...
    // Create window with medium blue background
    XColor blue_color;
    Colormap colormap = DefaultColormap(display, screen);
    XParseColor(display, colormap, "#4169E1", &blue_color); // Medium blue color
    XAllocColor(display, colormap, &blue_color);
    
    Window window = XCreateSimpleWindow(
        display, RootWindow(display, screen),
        10, 10, WINDOW_WIDTH, WINDOW_HEIGHT, 1,
        BlackPixel(display, screen), blue_color.pixel
    );
    ...
    while (running) {
        ...
        switch (event.type) {
            case Expose: {
                ...
                // Draw title
                XDrawString(display, window, gc, 200, 30, "Slides Transformation Tool", 26);
                XDrawString(display, window, gc, 200, 50, "Transforms eligible notes (SAN, RLN, SMP, I8, etc.) with slide patterns", 70);
                
                // Create light blue color for buttons
                XColor light_blue_color;
                XParseColor(display, colormap, "#87CEFA", &light_blue_color); // Light blue color
                XAllocColor(display, colormap, &light_blue_color);
                
                // Save original foreground color
                unsigned long original_fg = XGetForeground(display, gc);
                
                // Set light blue color for button backgrounds
                XSetForeground(display, gc, light_blue_color.pixel);
                
                // Draw buttons with light blue background
                ...
                // Draw slider with light blue background
                XSetForeground(display, gc, light_blue_color.pixel);
                ...
                // Draw slider handle with darker blue
                XColor dark_blue;
                XParseColor(display, colormap, "#1E90FF", &dark_blue);
                XAllocColor(display, colormap, &dark_blue);
                XSetForeground(display, gc, dark_blue.pixel);
                
                int sliderPos = 150 + (state.transformationPercentage * 2);
                XFillRectangle(display, window, gc, sliderPos, 140, 10, 20);
                
                // Draw variant selection with light blue background
                XSetForeground(display, gc, light_blue_color.pixel);
                ...
                // Draw process buttons with light blue background
                XSetForeground(display, gc, light_blue_color.pixel);
